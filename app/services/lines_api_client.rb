class LinesApiClient
  ...
  def self.ensure_second_half_lines_are_recent!
    if ENV["USE_ODDS_API"] == "1"
      ...
      if last_update.nil? || last_update < 1.minute.ago
        update_lines(:second_half)
      end
    end
  end

  def self.update_lines(scope = nil)
    new.update_lines(scope)
  end

  def update_lines(scope = nil)
    require 'open-uri'

    event_metadata = fetch_scoped_lines(scope: :game).compact
    if(scope == :first_half || scope == :second_half)
      fetch_scoped_lines(scope:, event_metadata:)
    end
  end

  def fetch_scoped_lines(scope: :game, event_metadata: nil)
  ...
  def parse_and_assert_lines(json, scope, activate_ids)
    Array.wrap(json).map do |event|
      home = Competitor.find_by_string!(event["home_team"], sport: :nfl)
      away = Competitor.find_by_string!(event["away_team"], sport: :nfl)
      starts_at = DateTime.parse(event["commence_time"])

      next if event["bookmakers"].blank?

      game = Game.nfl.find_or_create_by!(starts_at: starts_at, cached_competitor_ids: [away.id, home.id])
      ...
      { id: event["id"], starts_at: starts_at }
    end
  end
