namespace :scrape do
  ...
  task nfl_lines: :environment do
    ...
    game_elements.map do |ge|
      ...
      game.lines.point_spread.find_or_create_by!(contestant: away_contestant, value: away_spread) if away_spread.present?
      game.lines.point_spread.find_or_create_by!(contestant: home_contestant, value: home_spread) if home_spread.present?

      game.lines.moneyline.find_or_create_by!(contestant: away_contestant, value: 0, odds: away_ml) if away_ml.present?
      game.lines.moneyline.find_or_create_by!(contestant: home_contestant, value: 0, odds: home_ml) if home_ml.present?

      if total.present?
        game.lines.over.find_or_create_by!(value: total)
        game.lines.under.find_or_create_by!(value: total)
      end
